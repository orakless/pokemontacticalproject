...
#       * Fixed a bug where the modifier sequence system from 0.77.1 would prevent "after effects"
#         to take place (counterattack, timeline control effects, ect)
#       * Fixed a bug where statut icons would be incorrectly drawn
# - 0.77.2B
#       * Fixed a bug where state popups would overlap with damage ones
...
BattleManagerTBS.startTurn = function () {
	...
	for(let i=0; i<battlers.length; i++){
		var dist = LeUtilities.distanceBetweenCells(battlers[i].getCell(), entity.getCell());
            if (dist == 1 && LeUtilities.isEnemy(battlers[i].battler(), entity.battler()) && !battlers[i].battler().isDead()) {
            	entity._movePoints--; 
			}
		}
};
...
TBSEntity_Sprite.prototype.processBitmapsConfig = function (poseConfig, hue) {
    ...
    if (data.match(/blank_sprite/i)) {
        ...
        this._frameLoaded++;
        return;
    } else if (data.match(/singleton\,\s?(.+)/i)) {
        this._maxFrame[pose] = 0;
        this._nbrLines[pose] = 1;
        this._bitmaps[pose] = ImageManager.loadLeTBSBattler(RegExp.$1.trim(), hue);
        this._frameLoaded++;
        return;
    }
